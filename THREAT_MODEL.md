...
Attackers do not initially control trusted server configuration, policy bundles or host credentials. The inherited baseline corpus is shared; the protected lab adds server-owned tenant and grant checks, but production tenant provisioning remains deployment work. No general agent tool-execution authority was established in the baseline review; the protected lab only enables its synthetic, read-only mock tool.
...
The inherited answer cache uses exact hashed query keys, not semantic vector similarity (`backend/services/cache_service.py:51`, `backend/services/cache_service.py:84`). Chat cache keys use language and question with selected bypasses and no-history restrictions (`backend/routes/chat.py:95`, `backend/routes/chat.py:131`). Scope-aware caching is implemented in the protected gateway; the inherited cache remains outside that guarantee.
